...
In the 2018-2019 NBA regular season, we observed births of many new 3 pointers records. For example, Klay Thompson from the Golden State Warriors, made 14 3-pointers in Chicago last October. ([link](https://ftw.usatoday.com/2018/10/klay-thompson-14-3-pointers-nba-record)) James Harden, a key player from the Houston Rockets who is known for being deadly at making three pointers, created a record of second longest 30-point games streak, 32 games, in last February. This is the best record created after 1962. ([link](http://www.espn.com/nba/story/_/id/26081187/james-harden-historic-30-point-streak-ends-28-win-vs-atlanta)) Also, Klay Thompson mentioned earlier succeeded in getting 10 consecutive 3-pointers in last January. ([link](https://www.newsweek.com/klay-thompson-three-pointers-nba-records-golden-state-warriors-los-angeles-1299559)) 
...
We ran regressions on the team standings during the regular seasons by three pointer success rate and two pointer success rate. The depdent variable ia a team's standing during a given season and the indepdent variables are that team's three pointer success rate. We provided some variations of the regressions by looking at the success rate of two pointers and see how the inclusion of this indepdent variable may have on the regression results. If team has a lower standings, in another word, smaller number for its ranking position, it is ranked closer to the top so the better team. Unsurprisingly, the regressions showed that independent variables three pointer success rate and two pointer success rate all have negative coefficients and they are statistically significant at 5%. This means when the three pointer and two pointer success rates increase, the teams tend to have smaller numbers as their rankings, i.e., better rankings. 
...
Based on the charts illustrated below, we observed a few interesting points. Even though the three point success rate has stayed largely flat in recent years, as shown in chart Q2−1. 3 Pointer Field Goal made vs tries, teams on average have increased the number of three pointer attempts per game. This naturually led to an increase in the contribution of three pointers to total points scored by a team at the expense of two pointers, as shown in the chart Q2−2. Field Goal Percentage.
...
We have identified some well known players who excel at shooting three pointers. The graphs show each one of these top players' career-long progression of 3-pointer success rate. Though these players might have played for different teams and during time, we observed some interesting trends. First, three point shooting success rate generally go up at the begining, which is likely due to the fact that they received best in class training to help them improve their shooting skills when they first joined NBA. 
...
We wanted to explore whether players’ hometowns might have had any impact on their three point shooting skills because different regions may emphasize different basketball playing styles including those that emphasie shooting three pointers versus those that do not.
