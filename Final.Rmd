...
# To run this Rmarkdown fast, please download these data files from https://github.com/subeom/DECS-922-Final
#
#   1: dataGameLogsPlay.Rdata
#   2: dataGameLogsTeam.Rdata
#   3: df_nba_player_dics.Rdata
#   4: fgplayer.Rdata
#   5: nbaInsiderSalaries.Rdata
#
# If you don't have the Rdata files, the data will be downloaded from the Internet.
...
\medskip
```{r echo=FALSE}
fn <- c('dataGameLogsPlayer.Rdata', 'dataGameLogsTeam.Rdata', 'df_nba_player_dict.Rdata')

if (file.exists(fn[1]) && file.exists(fn[2]) && file.exists(fn[3])) {
  for (i in 1:length(fn)) {
    load(fn[i])
    print(paste0(fn[i], " loaded."))
  }
} else {
  library(nbastatR)
  library(future)
...
numberofplayerstate <- fgplayerhometown %>% group_by(State) %>% tally()

fgplayerhometownState <- fgplayerhometownState %>% left_join(numberofplayerstate, by=c("State"="State"))
fgplayerhometownState$state <- tolower(fgplayerhometownState$State)

states_map <- map_data("state") ## ggplot
states <- tibble(stateshort=state.abb, statelong=tolower(state.name))

mapState <- ggplot(fgplayerhometownState, aes(map_id=state)) +
  geom_map(aes(fill=pctfg3), map=states_map) +
...
fgplayerhometownState2 <- fgplayerhometownState %>% filter(n>=3)

mapState2 <- ggplot(filter(fgplayerhometownState2, pctfg3 > 30 && pctfg3 < 40), aes(map_id=state)) +
  geom_map(aes(fill=pctfg3), map=states_map) +
  coord_map("albers", at0 = 45.5, lat1 = 29.5) +
  expand_limits(x = states_map$long, y = states_map$lat) +
  scale_fill_gradientn(colours=c('red', 'yellow', 'green')) +
